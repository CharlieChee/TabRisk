#!/usr/bin/env python3
"""
Adult + DDPM LOO-MIA 后处理（端到端）。

与 adult_ctgan_loomia_postprocess 逻辑一致，区别：
- 仅扫描名称含 adult、control、ddpm（或 tabddpm）的 run
- Group A train_rows 仅 [200, 500, 1000, 1500]（无 2000）
- 5 个 seed（42–46）
- 图与 CSV 保存到同一风格目录，图中注明 DDPM、Adult 数据集
...
def parse_run_dir(dirname: str) -> Dict[str, Any]:
    ...
    目录名示例: train_adult_openml_..._train2000_synth2000_ddpm_1000iter_bs256_rounds20_candidate100_control_seed42_20260221_181549
    """
    out = {
        "train_rows": None,
        "n_iter": None,
...
def _process_one_run(run_dir_str: str) -> Optional[Dict[str, Any]]:
    """单 run 处理：解析 + AUC/CI + stability，返回 row 或 None。"""
...
def _is_ddpm_run(dirname: str) -> bool:
    """目录名是否对应 DDPM run（含 ddpm 或 tabddpm）。"""
    n = dirname.lower()
...
def scan_and_dedup(outputs_dir: Path, n_jobs: int = 1) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    """
    扫描 outputs 下含 shadow_pair_metrics 的 run，名称需含 adult、control、ddpm（或 tabddpm），
    ...
    for d in outputs_dir.iterdir():
        ...
        if "adult" not in name_lower or "control" not in name_lower:
            continue
        if not _is_ddpm_run(d.name):
            continue
...
def build_balanced_subsets(summary: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, Dict]:
    """
    Group A: n_iter=1000, rounds=20, candidate=100, train_rows in {200,500,1000,1500}, K=5 seeds (42–46).
    Group B/C 同 CTGAN 逻辑。
...
def fig1_adv_vs_train(scaling_balanced: pd.DataFrame, out_path: Path, jitter_seed: int = 42) -> None:
    """Fig 1: x=train_rows, y=adv_auc；标题注明 DDPM, Adult。"""
    ...
    ax.set_title(f"Adv(AUC) vs Train Size (Scaling) — {DATASET_MODEL_LABEL}")
...
def fig2_adv_vs_rounds(rounds_balanced: pd.DataFrame, out_path: Path, jitter_seed: int = 42) -> None:
    """Fig 2: x=rounds, y=adv_auc；标题注明 DDPM, Adult。"""
    ...
    ax.set_title(f"Adv(AUC) vs Rounds (Saturation) — {DATASET_MODEL_LABEL}")
...
def fig3_adv_vs_iter(iter_balanced: pd.DataFrame, out_path: Path, jitter_seed: int = 42) -> None:
    """Fig 3: x=n_iter, y=adv_auc；标题注明 DDPM, Adult。"""
    ...
    ax.set_ylabel("Adv(AUC)")
    ax.set_title(f"Adv(AUC) vs Training Iterations — {DATASET_MODEL_LABEL}")
...
def fig4_adv_vs_stability(scaling_balanced: pd.DataFrame, out_path: Path) -> None:
    """Fig 4: x=stability_metric, y=adv_auc；标题注明 DDPM, Adult。"""
    ...
    if x.size > 2 and np.isfinite(x).all() and np.isfinite(y).all():
        ...
        ax.set_title(
            f"Adv(AUC) vs Stability — {DATASET_MODEL_LABEL}\n"
...
def fig5a_learned_scaling(df_with_learned: pd.DataFrame, out_path: Path) -> None:
    """Fig 5A: learned_adv vs train_rows；标题注明 DDPM, Adult。"""
    ...
    ax.set_title(f"Learned Attacker vs Train Size — {DATASET_MODEL_LABEL}")
    ax.set_xticks(sorted(df["train_rows"].unique()))
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    out_path.parent.mkdir(parents=True, exist_ok=True)
...
def fig5b_learned_saturation(df_with_learned: pd.DataFrame, out_path: Path) -> None:
    """Fig 5B: learned_adv vs rounds；标题注明 DDPM, Adult。"""
    ...
    ax.set_xlabel("Shadow rounds per target")
    ax.set_ylabel("Learned attacker (5-fold CV AUC)")
    ax.set_title(f"Learned Attacker vs Rounds — {DATASET_MODEL_LABEL}")
    ax.set_xticks(sorted(df["rounds"].unique()))
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    out_path.parent.mkdir(parents=True, exist_ok=True)
...
def write_table1(scaling_balanced: pd.DataFrame, out_path: Path) -> None:
    """Table 1: 按 train_rows 聚合 mean adv_auc, std, 95% CI, n_seeds。"""
    df = scaling_balanced.dropna(subset=["train_rows", "adv_auc"]).copy()
    if df.empty:
        return
    rows = []
...
def write_report(
    ...
    """写入 postprocess_report.txt（注明 Adult+DDPM）。"""
    lines = [
        "Adult+DDPM LOO-MIA Postprocess Report",
        "======================================",
        f"Dataset / Model: {DATASET_MODEL_LABEL}",
        "",
        "Run completeness: pair_metrics.csv, pair_metrics_control.csv, mmd_component_analysis.json",
        "",
        "Scan & dedup (adult + control + ddpm/tabddpm):",
        f"  total_scanned_dirs: {scan_report.get('total_scanned_dirs', 0)}",
        f"  has_metrics_dir (adult+control+ddpm): {scan_report.get('has_metrics_dir', 0)}",
        ...
        "",
        "Balanced subsets (Group A train_rows = 200, 500, 1000, 1500 only):",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Adult+DDPM LOO-MIA 端到端后处理：总表、平衡子集、图、Table1、报告（图注明 DDPM, Adult）")
    parser.add_argument("--outputs", type=Path, default=PROJECT_ROOT / "outputs", help="outputs 根目录")
    parser.add_argument("--outdir", type=Path, default=PROJECT_ROOT / "outputs" / "postprocess_adult_ddpm", help="输出目录（与 CTGAN 同风格，CSV/图/报告）")
    parser.add_argument("--with-fig5", action="store_true", help="是否计算并绘制 Fig5 learned attacker")
    parser.add_argument("--n-jobs", type=int, default=1, help="并行进程数；1=单进程")
    ...
    # 1) 扫描 + 去重 → 总表（仅 adult + control + ddpm/tabddpm）
    summary, scan_report = scan_and_dedup(outputs_dir, n_jobs=args.n_jobs)
    outdir = args.outdir.resolve()
    outdir.mkdir(parents=True, exist_ok=True)

    summary_path = outdir / "adult_ddpm_run_summary.csv"
    if summary.empty:
        summary.to_csv(summary_path, index=False, encoding="utf-8-sig")
        print("未找到任何完整 DDPM run（adult+control+ddpm），已写出空总表。")
    ...
    # 2) 平衡子集（Group A 仅 200,500,1000,1500）
    ...
    # 3) 图（保存到 outdir/figures/，标题注明 DDPM, Adult）
