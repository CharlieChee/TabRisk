#!/usr/bin/env python3
"""
DPGAN 超参扫描脚本：16 组实验，最多 8 GPU 并行，打印 utility。
...
# Sweep 网格：bs=64,128,512,1024；iter=50,100,300,500。满足 batch_size <= train_rows/2 用 train_rows=2500
BATCH_SIZES = [64, 128, 512, 1024]
N_ITERS = [50, 100, 300, 500]
...
def run_one(bs: int, n_iter: int, gpu_id: int) -> dict:
    """运行单次 DPGAN 实验，返回 {bs, n_iter, gpu_id, output_dir, overall_score, error}。"""
    run_name = f"bs{bs}_iter{n_iter}"
    output_dir_rel = f"outputs/sweep_dpgan/{run_name}"

    # train_rows=2500 保证 1024 <= train_rows/2
    cmd = [
        "python",
        "-m",
        "synthgen.train",
