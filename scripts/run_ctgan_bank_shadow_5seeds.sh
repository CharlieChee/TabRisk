#!/usr/bin/env bash
# CTGAN shadow 实验（Bank 数据集）：5 个 train_rows × 5 seeds = 25 个 run，顺序执行，可 nohup 后台
#
# 配置：与 run_ddpm_shadow_5seeds 同逻辑，仅 data=standard/bank, model=ctgan
#   data=standard/bank, model=ctgan
...
#
# 用法：
#   chmod +x scripts/run_ctgan_bank_shadow_5seeds.sh
# 后台运行（推荐）：
#   cd /Users/changlong.ji/Desktop/project/TabRisk
#   nohup bash scripts/run_ctgan_bank_shadow_5seeds.sh > run_ctgan_bank_shadow_5seeds.log 2>&1 &
# 前台运行：
#   bash scripts/run_ctgan_bank_shadow_5seeds.sh

set -e
cd "$(dirname "$0")/.."
LOG_DIR="${LOG_DIR:-logs_ctgan_bank_shadow_5seeds}"
mkdir -p "$LOG_DIR"
...
echo "========== CTGAN (bank) shadow: train_rows=200, bs=128, 5 seeds =========="
for s in "${SEEDS[@]}"; do run_one 200 128 "$s"; done

echo "========== CTGAN (bank) shadow: train_rows=500, bs=256, 5 seeds =========="
for s in "${SEEDS[@]}"; do run_one 500 256 "$s"; done

echo "========== CTGAN (bank) shadow: train_rows=1000, bs=256, 5 seeds =========="
for s in "${SEEDS[@]}"; do run_one 1000 256 "$s"; done

echo "========== CTGAN (bank) shadow: train_rows=1500, bs=256, 5 seeds =========="
for s in "${SEEDS[@]}"; do run_one 1500 256 "$s"; done

echo "========== CTGAN (bank) shadow: train_rows=2000, bs=256, 5 seeds =========="
